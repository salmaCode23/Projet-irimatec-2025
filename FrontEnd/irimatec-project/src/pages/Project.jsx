...
export default function Project() {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:3000/api/projets")
      .then((res) => res.json())
      .then((data) => {
        setProjects(data);
        setLoading(false);
    ...
    <div>
      ...
      <div>
        ...
        <p className="text-[20px] md:px-16 mb-6 text-center px-7">
          « Chez <span className="text-green-700 font-bold">IRIMATEC</span>,
          nous réalisons des projets innovants alliant expertise technique et
          solutions sur-mesure pour répondre aux besoins spécifiques de nos
          clients, avec un engagement fort pour la qualité et la performance. »
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 md:justify-items-stretch m-4">
          {projects.map((project) => (
            <CardComponent
              ...
              image={`http://127.0.0.1:3000/uploads/${project.image}`}
              action={
